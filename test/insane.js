...
test('succeeds because of sensible defaults', function (t) {
  t.equal(
    insane('<div>bar<span>foo</span></div>'),
    '<div>bar<span>foo</span></div>'
  )
  t.end()
})
...
test('succeeds because of whitelist approach', function (t) {
  t.equal(
    insane('<script>bar<span>foo</span></script>', { allowedTags: [] }),
    ''
  )
  t.end()
})
...
test('only returns tags in the whitelist', function (t) {
  t.equal(
    insane('<p><span>foo</span>bar</p>', { allowedTags: ['p'] }, true),
    '<p>bar</p>'
  )
  t.equal(
    insane('<p>bar<span>foo</span></p>', { allowedTags: ['p'] }, true),
    '<p>bar</p>'
  )
  t.end()
})
...
test('only returns tags in the whitelist even if repeated', function (t) {
  t.equal(
    insane('<p><p>foo</p>bar</p>', { allowedTags: ['p'] }, true),
    '<p><p>foo</p>bar</p>'
  )
  t.equal(
    insane('<p><p><span>foo</span></p>bar</p>', { allowedTags: ['p'] }, true),
    '<p><p></p>bar</p>'
  )
  t.equal(
    insane('<p><span><p>foo</p></span>bar</p>', { allowedTags: ['p'] }, true),
    '<p>bar</p>'
  )
  t.end()
})
...
test('calls filter', function (t) {
  ...
  t.deepEqual(filter.firstCall.args, [
    { attrs: { a: 'a', b: 'b', class: 'foo bar' }, tag: 'div' },
  ])
  t.end()
})
...
test('succeeds to read urls that make sense', function (t) {
  ...
  t.equal(insane('<a href="/foo">bar</a>'), '<a href="/foo">bar</a>')
  t.equal(
    insane('<a href="http://google.com/foo">bar</a>'),
    '<a href="http://google.com/foo">bar</a>'
  )
  t.equal(
    insane('<a href="https://google.com/foo">bar</a>'),
...
test("fails to read urls that don't make sense", function (t) {
  ...
  t.equal(
    insane('<a href="magnet:?xt=urn:btih:E6462F43A9B7329961FADA1">bar</a>'),
    '<a>bar</a>'
  )
  t.end()
})
...
test('removes doctype', (t) => {
  const input = `<!DOCTYPE html><div>foo</div>`
  const output = `<div>foo</div>`

  t.equal(insane(input), output)
  t.end()
})
